...
'-----------------------------'
opakovani = 8
redim startarr(0 to opakovani) as integer 'pozice startu ve ctverici'
redim laserarr(0 to opakovani) as integer 'pozice znacek ve ctverici'
posun = 0
laser = 3
startarr(0)=1 
startarr(1)=6
...
startarr(6)=7
startarr(7)=1
for i = 0 to opakovani
  phases(p).cues(0).laserarr(i+posun) = startarr(i)  
  phases(p).cues(1).laserarr(i+posun) = laser
  phases(p).sectors(s).carr(i+posun) = (laser-1) * 45 + uhelzmena 'cil je vazan ke znacce' - we substract 1 from the position. because the round numbering starts at 1 - 1=0degrees, 2 = 45 degrees etc.
